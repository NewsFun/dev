<template>
  <div id="calculator" v-if="showCalculator" v-draggable="draggableValue">
    ...
    <div class="buttons" style="display:inline-block;">
      <div class="btn-group-justified">
        ...
        <button class="btn symbol" :class="{'disabled':bracesStatus==='right'}" @click="leftBraces">(</button>
        <button class="btn symbol" :class="{'disabled':bracesStatus==='left'}" @click="rightBraces">)</button>
      </div>
      ...
      <div class="btn-group-justified">
        ...
        <button class="btn number" @click="selectNum('.')"><strong>.</strong></button>
        <button class="btn symbol" @click="calculate('+')">+</button>
        <button class="btn memory" @click="equal">=</button>
      </div>
    </div>
    <div class="transfer">
      <div class="transfer-btn" :class="{'active':transferActive}" @click="transfer">Transfer Display</div>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    filters: {
      addDecimal(value) {
        ...
        const val = value.indexOf('.') > -1 ? value.toString() : value.toString() + '.';
        return val;
      },
      toFixed(value) {
        if (value === 'ERROR' || value === '(') return value;
      ...
      addComma(value) {
        ...
        let n = val.length % 3 === 0 ? Math.floor(val.length / 3) - 1 : Math.floor(val.length / 3);
        let arr = [];
        for (let i = n; i > 0; i--) {
    ...
    methods: {
      ...
      calculate(type) {
        ...
        if (this.openLeftBraces) {
          if (!this.bracesCalculateType) {
            ...
            if (type === '*' || type === '/') {
              if (this.bracesString.indexOf('+') === -1 && this.bracesString.indexOf('-') === -1) {
                this.result = math.parser().eval(this.bracesString).toString();
              }
            }
          } else {
            this.bracesString = this.bracesString.substr(0, this.bracesString.length - 1);
          }
        ...
        if (!this.calculateType) {
          ...
          if (type === '+' || type === '-') {
            this.result = math.parser().eval(this.calString).toString();
          } else if (type === '*' || type === '/') {
            this.calString = this.result;
          }
        ...
        if (this.firstParams && (type === '+' || type === '-')) {
          ...
          if (this.calString.indexOf('+') === -1 && this.calString.indexOf('-') === -1) {
            this.result = math.parser().eval(this.calString).toString();
          } else {
            const num = this.calString.indexOf('-') > -1 ? this.calString.indexOf('-') : this.calString.indexOf('+');
            const str = this.calString.substr(num + 1, this.calString.length);
            this.result = math.parser().eval(str).toString();
          }
        }
      ...
      equal() {
        ...
        this.calculateType = null;
        if (this.openLeftBraces) {
          this.bracesString = this.addResult ? this.bracesString += this.result : this.bracesString.substr(0, this.bracesString.length - 1);
          this.bracesString = math.parser().eval(this.bracesString);
          this.calString += this.bracesString;
          const result = math.parser().eval(this.calString).toString();
        ...
        // console.log(this.calString);
        if (this.calString.endsWith('*') || this.calString.endsWith('/') || this.calString.endsWith('+') || this.calString.endsWith('-')) {
          this.calString = this.calString.substr(0, this.calString.length - 1);
        }
        const result = this.calString === '' ? '0' : math.parser().eval(this.calString).toString();
        this.result = this.checkValid(result);
        this.calString = '';
        this.addResult = true;
        this.addPlusMinus = true;
      ...
      rightBraces() {
        ...
        if (this.bracesString.endsWith('*') || this.bracesString.endsWith('/') || this.bracesString.endsWith('+') || this.bracesString.endsWith('-')) {
          this.bracesString = this.bracesString.substr(0, this.bracesString.length - 1);
        }
        const result = math.parser().eval(this.bracesString).toString();
        this.result = this.checkValid(result);
        this.bracesString = '';
      },
      // 修改正负号
      plusMinus() {
        if (this.result === 'ERROR' || !parseFloat(this.result) || !this.addPlusMinus) return;
      ...
      checkValid(val) {
        ...
        if (parseFloat(val) > -0.0000001 && parseFloat(val) < 0.0000001) return '0';
        return val;
      },
      showMemory() {
      ...
      // 清空输入数字
      clearPart() {
        if (this.result === 'ERROR' || this.calculateType || this.bracesCalculateType) return;
        this.lastParams = '';
        this.result = '0';
        this.addResult = false;
      },
